...
"""
    ...
    def setup_parameters_panel(self, parent):
        ...
        # Random map button
        self.random_map_button = ttk.Button(params_frame, text="🎲 Générer Carte Aléatoire",
        ...
        # Discount factor
        gamma_label = ttk.Label(params_frame, text="Facteur de discount (γ):")
        gamma_label.grid(row=row, column=0, sticky=tk.W, pady=5)
        ToolTip(gamma_label, "Importance du futur\n\n"
                "ÉLEVÉ (0.95-0.99): Planifie à long terme\n"
                "BAS (0.5-0.8): Myope, récompense immédiate\n\n"
        ...
        # Epsilon decay
        decay_label = ttk.Label(params_frame, text="Décroissance epsilon:")
        decay_label.grid(row=row, column=0, sticky=tk.W, pady=5)
        ToolTip(decay_label, "Vitesse de passage exploration→exploitation\n\n"
                "RAPIDE (0.95-0.98): Arrête d'explorer vite\n"
                "LENT (0.995-0.999): Continue d'explorer\n\n"
                "💡 Recommandé: 0.995-0.997")
        ...
        # Episodes
        ep_label = ttk.Label(params_frame, text="Nombre d'épisodes:")
        ep_label.grid(row=row, column=0, sticky=tk.W, pady=5)
        ToolTip(ep_label, "Nombre de parties d'entraînement\n\n"
                "COURT (<5000): Rapide mais peu efficace\n"
                "MOYEN (10000): Bon équilibre\n"
        ...
        # Update frequency
        update_label = ttk.Label(params_frame, text="Mise à jour UI:")
        update_label.grid(row=row, column=0, sticky=tk.W, pady=5)
        ToolTip(update_label, "Fréquence de rafraîchissement de l'interface\n\n"
                "BAS (10-50): Plus fluide, peut ralentir\n"
                "MOYEN (100): Bon équilibre\n"
        ...
        # Preset buttons
        preset_label = ttk.Label(params_frame, text="🎯 Préréglages:", font=('Arial', 10, 'bold'))
        preset_label.grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))
        row += 1

        beginner_btn = ttk.Button(params_frame, text="🎓 Débutant (Facile, rapide)",
        ...
        standard_btn = ttk.Button(params_frame, text="⚡ Standard (Équilibré)",
        ...
        optimal_btn = ttk.Button(params_frame, text="🎯 Optimal (Meilleure perf)",
        ...
        ttk.Label(params_frame, text="🎮 Mode Ludique", font=('Arial', 11, 'bold')).grid(
            row=row, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))
        row += 1

        # Action weights panel
        action_frame = ttk.LabelFrame(params_frame, text="⚡ Poids des Actions", padding="15")
        ...
        ttk.Label(action_frame, text="← Gauche:", font=('Arial', 9)).grid(row=0, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        ToolTip(left_spin, "Poids pour aller à gauche\n>1.0 = favorisé\n<1.0 = défavorisé")

        ttk.Label(action_frame, text="↓ Bas:", font=('Arial', 9)).grid(row=1, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        ttk.Label(action_frame, text="→ Droite:", font=('Arial', 9)).grid(row=2, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        ToolTip(right_spin, "Poids pour aller à droite\n>1.0 = favorisé\n<1.0 = défavorisé")

        ttk.Label(action_frame, text="↑ Haut:", font=('Arial', 9)).grid(row=3, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        self.action_prob_label = ttk.Label(action_frame, text="Probabilités: ← 25% | ↓ 25% | → 25% | ↑ 25%",
                                           font=('Arial', 8), foreground='blue')
        self.action_prob_label.grid(row=4, column=0, columnspan=2, pady=(8, 0))
        ToolTip(self.action_prob_label, "Probabilités relatives pendant l'exploration aléatoire")
        ...
        reward_frame = ttk.LabelFrame(params_frame, text="🎁 Récompenses Personnalisées", padding="15")
        ...
        ttk.Label(reward_frame, text="🏆 Objectif atteint:", font=('Arial', 9)).grid(
        ...
        ToolTip(goal_spin, "Récompense pour atteindre le but\nDéfaut: 1.0\nPlus haut = plus motivé")

        ttk.Label(reward_frame, text="💀 Tombe dans trou:", font=('Arial', 9)).grid(
        ...
        ToolTip(hole_spin, "Pénalité pour tomber dans un trou\nDéfaut: 0.0\nNégatif = pénalise plus")

        ttk.Label(reward_frame, text="👣 Chaque pas:", font=('Arial', 9)).grid(
        ...
        ToolTip(step_spin, "Récompense/pénalité par pas\nDéfaut: 0.0\nNégatif = encourage rapidité")

        # Advanced penalties panel
        penalties_frame = ttk.LabelFrame(params_frame, text="⚙️ Pénalités Avancées", padding="15")
        ...
        ttk.Label(penalties_frame, text="🧱 Collision mur:", font=('Arial', 9)).grid(
            row=0, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        ToolTip(wall_spin, "Pénalité quand l'agent reste sur place (mur)\nDéfaut: 0.0\nNégatif = pénalise les collisions")

        ttk.Label(penalties_frame, text="🔁 Revisite case:", font=('Arial', 9)).grid(
            row=1, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        ToolTip(revisit_spin, "Pénalité pour visiter une case déjà visitée\nDéfaut: 0.0\nNégatif = encourage exploration")

        ttk.Label(penalties_frame, text="🔄 Boucle détectée:", font=('Arial', 9)).grid(
            row=2, column=0, sticky=tk.W, pady=3, padx=(0, 10))
        ...
        ToolTip(loop_spin, "Pénalité pour boucler (même case dans les 5 derniers pas)\nDéfaut: 0.0\nNégatif = pénalise les boucles")

        ttk.Label(penalties_frame, text="📏 Distance au but:", font=('Arial', 9)).grid(
        ...
        ToolTip(distance_spin, "Bonus basé sur la distance au but\nDéfaut: 0.0\nPositif = récompense rapprochement\nNégatif = pénalise éloignement")
    ...
    def reset_ludic_mode(self):
        ...
        self.log_message("✅ Mode ludique réinitialisé aux valeurs par défaut")
    ...
    def show_learning_curve(self):
        ...
        try:
            ...
            ax1.set_title(f'Taux de Réussite (moy. {window_size})', fontsize=11, fontweight='bold')
            ...
            ax2.set_ylabel('Résultat', fontsize=10)
            ax2.set_title('Résultats par Épisode', fontsize=11, fontweight='bold')
            ax2.set_yticks([0, 100])
            ax2.set_yticklabels(['Échec', 'Succès'], fontsize=8)
    ...
    def setup_visualization_panel(self, parent):
        ...
        live_check = ttk.Checkbutton(control_frame, text="Voir entraînement",
                                     variable=self.show_live_grid)
        live_check.grid(row=0, column=6, padx=(20, 5))
        ToolTip(live_check, "Affiche l'agent dans la grille pendant l'entraînement")
        ...
        # Win rate
        win_label = ttk.Label(stats_container, text="Taux de réussite:", font=('Arial', 10, 'bold'))
        ...
        # Average reward
        ttk.Label(stats_container, text="Récompense moyenne:",
    ...
    def on_map_size_change(self, event=None):
        ...
        if self.map_size.get() == "Personnalisée":
    ...
    def generate_random_map(self):
        ...
        try:
            ...
            def on_generate():
                size = size_var.get()
                prob = prob_var.get()
                self.log(f"\n🎲 Génération d'une carte {size}x{size} (probabilité trous: {prob:.2f})...")
